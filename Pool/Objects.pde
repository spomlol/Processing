...
class otherball {
  ...
  void update() {
    ...
    //if (dist(position.x,position.y,ball.x,ball.y)<=30){
    //  float angle = atan2(-ball.y+position.y,-ball.x+position.x);
    ...
    //  //ball.vx = newV2.x;
    //  //ball.vy = newV2.y;
    //  velocity.x=0.9*cos(angle)*sqrt(ball.vx * ball.vx + ball.vy * ball.vy) + 0.1*cos(angle)*sqrt(tvx * tvx + tvy * tvy);
    //  velocity.y=0.9*sin(angle)*sqrt(ball.vx * ball.vx + ball.vy * ball.vy) + 0.1*sin(angle)*sqrt(tvx * tvx + tvy * tvy);

    //  ball.vx = -0.9*cos(angle)*sqrt(tvx * tvx + tvy * tvy) - 0.1*cos(angle)*sqrt(ball.vx * ball.vx + ball.vy * ball.vy);
    //  ball.vy = -0.9*sin(angle)*sqrt(tvx * tvx + tvy * tvy) - 0.1*sin(angle)*sqrt(ball.vx * ball.vx + ball.vy * ball.vy);

    //   if (dist(position.x,position.y,ball.x,ball.y)<30) {
    //     ball.x=-cos(angle)*30+position.x;
    //     ball.y=-sin(angle)*30+position.y;
    //   }
    //}      
